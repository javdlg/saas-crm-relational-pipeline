...
def process_and_load():
    ...
    for _, row in companies_df.iterrows():
        ...
        company = Company(
            ...
            frequency_of_purchase=str(row['Frequency_of_Purchase']).strip() if pd.notna(row['Frequency_of_Purchase']) else None,
            days_since_last_purchase=safe_int(row['Days_Since_Last_Purchase']),
            contract_status=str(row['Contract_Status']).strip() if pd.notna(row['Contract_Status']) else None,
    ...
    for _, row in employees_df.iterrows():
        ...
        employee = Employee(
            ...
            decision_maker_flag=str(row['Decision_Maker_Flag']).strip() if pd.notna(row['Decision_Maker_Flag']) else None,
            ...
            active_flag=str(row['Active_Flag']).strip() if pd.notna(row['Active_Flag']) else None,
